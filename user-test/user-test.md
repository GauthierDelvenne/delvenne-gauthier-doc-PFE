---
layout: page
title: "User Test"
---

[← Retour au sommaire]({{ site.baseurl }}/)
# User Test

## Profil des testeurs
- **Testeur 1 (Chloé, 23 ans)** : Manque de temps au quotidien, n’aime pas faire les magasins ni y perdre du temps. À l’aise avec la technologie.
- **Testeur 2 (Jean, 26 ans)** : Très à l’aise avec la technologie. Adepte des commandes en ligne.
- **Testeur 3 (Marie, 51 ans)** : Peu à l’aise avec la technologie, commande en ligne de temps en temps et a ses habitudes.

---

## T1

**Cible** : Client

**Tâche** : Rechercher un produit alimentaire, utiliser les filtres du catalogue et sélectionner un produit.

**Objectif** : Trouver rapidement un produit, sans frustration.

### Testeur 1
**Point fort** : Trouve rapidement les produits alimentaires depuis la page d’accueil. Elle utilise le filtre « Catégorie(s) » pour affiner sa recherche.

**Faiblesse** : Elle confond les filtres de tri et les filtres de sélection. Elle n’a pas utilisé les catégories disponibles sur la page d’accueil.

[▶️ Voir la vidéo](/assets/videos/test-chloe/t1-chloe.mp4)

### Testeur 2
**Point fort** : Trouve rapidement les articles et utilise la navigation pour accéder directement au catalogue.

**Faiblesse** : /

[▶️ Voir la vidéo](/assets/videos/test-jean/t1-jean.mp4)
### Testeur 3
**Point fort** : Utilise efficacement les moyens de navigation ainsi que les filtres pour trouver les produits recherchés.

**Faiblesse** : Désactive puis réactive le filtre, ce qui entraîne une manipulation inutile et peut ralentir la recherche.

[▶️ Voir la vidéo](/assets/videos/test-marie/t1-marie.mp4)

### Conclusion et bilan T1
Les utilisateurs utilisent différents moyens pour accéder aux produits alimentaires. Ils comprennent le fonctionnement des filtres et trouvent rapidement le produit recherché. Cependant, certains peuvent se perdre dans les filtres en raison de l’espacement et de l’organisation de ceux-ci.

---

## T2

**Cible** : Client

**Tâche** : Créer un compte client, composer un panier multi-commerçants et valider son panier avec un créneau.

**Objectif** : Lever les freins à l’inscription et centraliser les achats.

### Testeur 1
**Point fort** : Inscription rapide. Une fois le fonctionnement compris, l’ajout de produits est efficace. Le choix du créneau se fait sans encombre, avec un récapitulatif clair des détails.

**Faiblesse** : N’a pas utilisé l’icône pour se connecter, doute sur le processus d’inscription et rencontre des difficultés lors de l’ajout au panier.

[▶️ Voir la vidéo](/assets/videos/test-chloe/t2-chloe.mp4)

### Testeur 2
**Point fort** : Choix du créneau efficace. L’utilisateur voit rapidement qui a vendu quoi, même sans utiliser les filtres.

**Faiblesse** : N’a pas vu les critères du mot de passe, n’utilise pas le bouton de connexion/inscription dans la navigation et rencontre des problèmes lors de l’ajout au panier.

[▶️ Voir la vidéo](/assets/videos/test-jean/t2-jean.mp4)
### Testeur 3
**Point fort** : Utilise la navigation pour s’inscrire.

**Faiblesse** : Critères du mot de passe non lus, ne trouve pas le panier.

[▶️ Voir la vidéo](/assets/videos/test-marie/t2-marie.mp4)

### Conclusion et bilan T2
La création d’une commande se fait assez rapidement, surtout dans les dernières étapes. En revanche, des difficultés apparaissent au début, notamment lors de la création du compte en raison de critères de mot de passe pas assez visibles, ainsi que lors de l’ajout de produits au panier.

---

## T3

**Cible** : Client

**Tâche** : Se rendre sur son espace personnel, consulter l’historique de ses commandes et vérifier les détails de celles-ci.

**Objectif** : Permettre au client de retrouver facilement ses informations de retrait (créneau, prix, produits) sans passer trop de temps à chercher.

### Testeur 1
**Point fort** : Clique rapidement aux bons endroits et parvient vite à destination.

**Faiblesse** : /

[▶️ Voir la vidéo](/assets/videos/test-chloe/t3-chloe.mp4)

### Testeur 2
**Point fort** : Va directement au bon endroit et trouve rapidement l’information.

**Faiblesse** : /

[▶️ Voir la vidéo](/assets/videos/test-jean/t3-jean.mp4)
### Testeur 3
**Point fort** : Trouve rapidement les commandes et le détail.

**Faiblesse** : /

[▶️ Voir la vidéo](/assets/videos/test-marie/t3-marie.mp4)

### Conclusion et bilan T3
Une tâche est réalisée rapidement, sans que l’utilisateur ne se perde.

---

## T4

**Cible** : Futur commerçant

**Tâche** : Découvrir ce que font les commerçants et soumettre une demande d’adhésion via la page « Commerçants ».

**Objectif** : Formulaire d’acquisition simple, court et rassurant.

### Testeur 1
**Point fort** : Utilise les différents boutons de navigation, formulaire rapide et constate que le formulaire a bien été envoyé.

**Faiblesse** : Dans l’accordéon, elle clique sur le nom pour développer la carte.

[▶️ Voir la vidéo](/assets/videos/test-chloe/t4-chloe.mp4)

### Testeur 2
**Point fort** : Utilise les flèches de l’accordéon, voit le message de succès et remplit le formulaire sans encombre.

**Faiblesse** : Remonte pour utiliser le bouton qui le renvoie au formulaire.

[▶️ Voir la vidéo](/assets/videos/test-jean/t4-jean.mp4)
### Testeur 3
**Point fort** : Lecture des informations avant d’accéder au formulaire.

**Faiblesse** : Se connecte, n’a pas vu le message de succès.

[▶️ Voir la vidéo](/assets/videos/test-marie/t4-marie.mp4)

### Conclusion et bilan T4
L’utilisateur trouve facilement le formulaire et les différentes informations, mais clique parfois au hasard à la recherche d’une interaction.

---

## T5

**Cible** : Commerçant

**Tâche** : Ajouter un nouveau produit et consulter le détail du produit.

**Objectif** : Proposer une interface administrative minimaliste, rapide et efficace.

### Testeur 1
**Point fort** : Utilise le bouton dans la barre latérale et applique efficacement les filtres.

**Faiblesse** : Actions effectuées trop rapidement pour le système, pouvant entraîner des lenteurs ou des erreurs d’affichage.

[▶️ Voir la vidéo](/assets/videos/test-chloe/t5-chloe.mp4)

### Testeur 2
**Point fort** : Ajout du produit rapide via la barre latérale (sidebar).

**Faiblesse** : Ne voit pas son produit après l’ajout et le recherche ensuite.

[▶️ Voir la vidéo](/assets/videos/test-jean/t5-jean.mp4)
### Testeur 3
**Point fort** : Utilise la barre latérale (sidebar) ainsi que le champ de recherche.

**Faiblesse** : Cherche directement dans les catégories.

[▶️ Voir la vidéo](/assets/videos/test-marie/t5-marie.mp4)

### Conclusion et bilan T5
L’ajout d’un produit se fait rapidement et sans difficulté, mais l’utilisateur ressent une frustration lorsqu’il ne voit pas immédiatement le produit qu’il vient de créer.